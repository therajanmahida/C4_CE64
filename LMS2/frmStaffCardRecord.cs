...
{
	public partial class frmStaffCardRecord
	{
		...
		public void fillDepartment()
		{
			try {
				...
				adp.SelectCommand = new OleDbCommand("SELECT distinct RTRIM(Department) FROM Cards_Staff,Staff where Staff.StaffID=Cards_Staff.StaffID", CN);
				ds = new DataSet("ds");
				adp.Fill(ds);
				dtable = ds.Tables[0];
		...
		private void txtStaffName_TextChanged(System.Object sender, System.EventArgs e)
		{
			try {
				...
				con.Open();
				cmd = new OleDbCommand("select Staff.StaffID,StaffName,Status from Cards_Staff,Staff where Cards_Staff.StaffID=Staff.StaffID and Staffname like '" + txtStaffName.Text + "%' order by StaffName ", con);
		...
		private void cmbDepartment_SelectedIndexChanged(System.Object sender, System.EventArgs e)
		{
			try {
				con = new OleDbConnection(cs);
				con.Open();
				cmd = new OleDbCommand("select Staff.StaffID,StaffName,Status from Cards_Staff,Staff where Cards_Staff.StaffID=Staff.StaffID and Department='" + cmbDepartment.Text + "' order by StaffName ", con);
				rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
				DataGridView1.Rows.Clear();
				while ((rdr.Read() == true)) {
					DataGridView1.Rows.Add(rdr[0], rdr[1], rdr[2]);
