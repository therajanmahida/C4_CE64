...
{
	public partial class frmStudentsNoDuesdRecord
	{
		...
		private void frmNoDues_Student_Load(System.Object sender, System.EventArgs e)
		{
			fillSession();
		}
		...
		private void cmbCourse_SelectedIndexChanged(System.Object sender, System.EventArgs e)
		{
			try {
				...
				con.Open();
				string ct = "select distinct RTRIM(Department) from Student,NoDues_Student where Student.StudentID=NoDues_Student.StudentID and course = '" + cmbCourse.Text + "'";
		...
		private void cmbSession_SelectedIndexChanged(System.Object sender, System.EventArgs e)
		{
			try {
				...
				con = new OleDbConnection(cs);
				con.Open();
				string ct = "select distinct RTRIM(Course) from Student,NoDues_Student where Student.StudentID=NoDues_Student.StudentID and Stu_Session= '" + cmbSession.Text + "'";
				cmd = new OleDbCommand(ct);
				cmd.Connection = con;
				rdr = cmd.ExecuteReader();
				while (rdr.Read()) {
		...
		private void txtStudentName_TextChanged(System.Object sender, System.EventArgs e)
		{
			try {
				con = new OleDbConnection(cs);
				con.Open();
				cmd = new OleDbCommand("select Student.StudentID,StudentName,Status from NoDues_Student,Student where NoDues_Student.StudentID=Student.StudentID and Studentname like '" + txtStudentName.Text + "%' order by StudentName ", con);
				rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
				DataGridView1.Rows.Clear();
				while ((rdr.Read() == true)) {
					DataGridView1.Rows.Add(rdr[0], rdr[1], rdr[2]);
		...
		public frmStudentsNoDuesdRecord()
		{
			Load += frmNoDues_Student_Load;
			InitializeComponent();
		}
	}
}
