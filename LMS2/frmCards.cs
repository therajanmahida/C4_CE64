	...
	public partial class frmCards
	{
		...
		private void btnUpdate_record_Click(System.Object sender, System.EventArgs e)
		{
			...
			try {
				...
				for (int i = listView1.Items.Count - 1; i >= 0; i += -1) {
					...
					string cd = "update Cards_Student set Status='" + txtStatus.Text + "' where StudentID= '" + listView1.Items[i].SubItems[0].Text + "'";
		...
		private void btnUpdate1_Click(System.Object sender, System.EventArgs e)
		{
			...
			try {
				...
				for (int i = ListView2.Items.Count - 1; i >= 0; i += -1) {
					...
					string cd = "update Cards_Staff set Status='" + txtStatus1.Text + "' where StaffID= '" + ListView2.Items[i].SubItems[0].Text + "'";
					cmd = new OleDbCommand(cd);
					cmd.Connection = con;
					con.Open();
					cmd.ExecuteNonQuery();
		...
		private void Button8_Click(System.Object sender, System.EventArgs e)
		{
			...
			try {
				...
				con = new OleDbConnection(cs);
				con.Open();
				cmd = new OleDbCommand("select Staff.StaffID,StaffName,Status from Cards_Staff,Staff where Cards_Staff.StaffID=Staff.StaffID and Department= '" + cmbDepartment1.Text + "' order by StaffName", con);
				rdr = cmd.ExecuteReader();
				while (rdr.Read()) {
					var item = new ListViewItem();
					item.Text = rdr[0].ToString().Trim();
